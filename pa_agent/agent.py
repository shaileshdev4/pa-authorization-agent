...
root_agent = Agent(
    ...
    instruction=(
        ...
        "  - patient_age and patient_sex if available\n"
        "  - country_preference: 'United States'\n\n"
        "STEP 4: Call GenerateClinicalJustification:\n"
        "  - patient_data: JSON built from message context\n"
        "  - raw_clinical_context: build this string yourself by concatenating:\n"
        "    1. The full original user message text\n"
        "    2. PLUS every string in the 'clinical_notes_text' array from GetPatientData result\n"
        "    Join with '\\n\\n'. This combined string is raw_clinical_context.\n"
        "    This is mandatory — do not pass only the user message.\n"
